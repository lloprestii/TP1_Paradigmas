...
Magos::Magos(string nombre, int vida, int mana, int armadura, pair<unique_ptr<Arma>, unique_ptr<Arma>> armas) :
    nombre(nombre),
    vida(vida),
    mana(mana),
    armadura(armadura),
...
const pair<unique_ptr<Arma>, unique_ptr<Arma>>& Magos::get_armas() const {
    return armas;
}

void Magos::set_armas(pair<unique_ptr<Arma>, unique_ptr<Arma>> nuevas_armas) {
